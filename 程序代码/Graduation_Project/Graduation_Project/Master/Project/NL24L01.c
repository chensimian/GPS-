...
extern unsigned char NRF_Tx_data_OK;
...
//	GPIO_SetBits(GPIOB,NRF_CE_GPIO);			//发送数据
}


//向发送缓冲区写入数据并发送出去
char NRF_WRITE_TX_DATA(char *buf)
{
	unsigned char state;
...
void NRF_mode_init(void)
{
	...
	NRF_REG_WRITE_BYTE(RX_PW_P0+NRF_WRITE_REG,8);//通道0的有效数据宽度为8byte
